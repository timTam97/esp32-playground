...
A local MJPEG camera webpage for the Freenove ESP32-WROVER, with resolution,
JPEG compression controls, and live performance readings. The tested unit has
an **OV3660** (sensor PID `0x3660`); older project notes identified it as an OV2640.

This sketch stays awake to serve the webpage. Cloud captures and alerts are
provided by the separate [garage monitor](../garage_monitor/README.md).
...
Wi-Fi conditions affect the result. See [VALIDATION.md](VALIDATION.md)
...
4. Open the IP address printed on the serial console at 115200 baud from a browser
   on the same network. Use **http://CAMERA_IP/** with the address from serial;
   DHCP can change it after reconnecting.
